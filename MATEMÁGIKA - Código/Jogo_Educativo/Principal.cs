...
namespace Jogo_Educativo
{
    public partial class Principal : Form
    {
        ...
        protected void pegarIp()
        {
            ...
            this.ipAddress = ipHostInfo.AddressList.FirstOrDefault(a => a.AddressFamily == AddressFamily.InterNetwork);
        ...
        // Start waiting - espera dados do cliente
        ...
        protected void aceitarConexoes()
        {
            while (true)
            {
                ...
                {
                    //aceitar requisicoes
                    byte[] comingDataFromClient = new byte[1024 * 50000];

                    sock_Servidor.Listen(10);
                    Socket handler;
                    try
                    {
                        handler = sock_Servidor.Accept();
                    }
                    catch (Exception)
                    {
                        return;
                    ...
                    //receber nome usuario, level OU resposta
                    String msg;
                    int bytesRec1 = handler.Receive(comingDataFromClient);
                    msg = Encoding.ASCII.GetString(comingDataFromClient, 0, bytesRec1);

                    if (msg.Substring(0, 3) == "con")
                    {
                        //receber ip
                        String strAd;
                        int bytesRec2 = handler.Receive(comingDataFromClient);
                        strAd = Encoding.ASCII.GetString(comingDataFromClient, 0, bytesRec2);
                        IPAddress ad = IPAddress.Parse(strAd); //cria o ip para o usuario que mandou

                        //perguntar ao usuario se ele quer jogar... (opcoes Sim ou Nao)
                    ...
                    {
                        //usuario respondeu mensagem, com 'S' ou 'N'
                        if (msg.Substring(0, 1) == "S" || msg.Substring(0, 1) == "s")
                        {
                            this.jogMultiplayer = true;
                            int lv = Convert.ToInt32(msg.Substring(1));
                            this.pegarLeveisNoBD();
                            this.desmostrarBtnUsuariosDispon();
                            this.iniciarLevel();
                        }
                        else
                        if (msg.Substring(0, 1) == "N" || msg.Substring(0, 1) == "n")
                        {
                            //avisar o usuario que o outro usuario nao aceitou jogar
        ...
        */
        ...
        protected void mostrarUsuariosDispon()
        {
            ...
            this.lbTitulo.Location = new Point(190, this.lbTitulo.Location.Y);
        ...
        protected void btnJogarContraAmigoEspecifico_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                int iPortNo = System.Convert.ToInt16(this.portasUsDispon[tag]);
                ...
                {
                    //enviar requisicao
                    Object objRequisicao = "requisicao";
                    byte[] byRequisicao = System.Text.Encoding.ASCII.GetBytes(objRequisicao.ToString());

                    try
                    {
                        m_clientSocket.Send(byRequisicao);
                    }
                    catch (SocketException se)
                    {
                        string str;
        ...
        protected void comecarExplicacao()
        {
            ...
            if (this.etapa != 6) //etapa: explicacao do jogo
            {
                this.etapa = 3;
        ...
        protected void acabarQuiz(bool acertou)
        {
            ...
            //quiz no final de cada level
            {
                ...
                {
                    MessageBox.Show("VOCÊ GANHOU DO EU AMIGO!! Muito beem!");
